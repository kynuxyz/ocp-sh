...
yum install -y  wget git zile nano net-tools docker-ce\
...
[ ! -d openshift-ansible ] && git clone https://github.com/abn/openshift-ansible.git

cd openshift-ansible && git fetch && git checkout release-3.9-patched && cd ..
